  ...
  return (
    <div style={{
      ...
      {(() => {
        ...
        if (!entry || entry.ganancia === undefined) return null;
        const g = entry.ganancia as number;
        const color = g >= 0 ? 'var(--up)' : 'var(--down)';
        return (
          <p style={{ color, marginTop: 6, paddingTop: 6, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', gap: 12, fontWeight: 700, fontSize: 12 }}>
...
// Construye stops de gradiente: cada segmento es verde u rojo según ganancia del mes
function buildGradientStops(chartData: ReturnType<typeof buildData>) {
  if (chartData.length < 2) return [{ offset: '0%', color: '#00d4c2', opacity: 0.25 }];
  const stops: { offset: string; color: string; opacity: number }[] = [];
  const n = chartData.length;
  chartData.forEach((row, i) => {
    const pct = (i / (n - 1)) * 100;
    const color = (row.ganancia ?? 0) >= 0 ? '#22d3a0' : '#f43f5e';
    const op = 0.22;
    if (i > 0) stops.push({ offset: `${pct.toFixed(1)}%`, color, opacity: op });
    stops.push({ offset: `${pct.toFixed(1)}%`, color, opacity: op });
  });
  return stops;
}

export default function EvolucionChartA({ data, tenenciasPorMes, hideValues }: Props) {
  const chartData = buildData(data, tenenciasPorMes);
  const [isMobile, setIsMobile] = useState(false);
  const gradId = useId().replace(/:/g, '');

  const stops = buildGradientStops(chartData);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 10, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--muted)' }}>
          Relleno verde = mes con ganancia · rojo = mes con pérdida
        </span>
      </div>
      <div style={{ flex: 1, minHeight: isMobile ? 0 : 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 5, right: isMobile ? 8 : 45, left: isMobile ? 0 : 10, bottom: 0 }}>
            <defs>
              <linearGradient id={`gradDyn${gradId}`} x1="0" y1="0" x2="1" y2="0">
                {stops.map((s, i) => (
                  <stop key={i} offset={s.offset} stopColor={s.color} stopOpacity={s.opacity} />
                ))}
              </linearGradient>
              <linearGradient id={`gradAportes${gradId}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <Area yAxisId="usd" type="monotone" dataKey="acumulado" name="Aportes Acumulados" stroke="#3b82f6" strokeWidth={2} fill={`url(#gradAportes${gradId})`} dot={false} activeDot={{ r: 5, fill: '#3b82f6', strokeWidth: 0 }} />
            <Area yAxisId="usd" type="monotone" dataKey="total_cartera" name="Total Cartera" stroke="#00d4c2" strokeWidth={2.5} fill={`url(#gradDyn${gradId})`} dot={false} activeDot={{ r: 5, fill: '#00d4c2', strokeWidth: 0 }} />
