...
import type { DashboardData, GrupoBono, BondPerformance } from '@/types';
...
export default function PerformanceTab({ data }: Props) {
  ...
  const tenencias = useMemo(() => {
    ...
    return tickersDeCartera(tenenciasPorMes[ultimoMes] ?? []).tenencias;
  }, [tenenciasPorMes]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1, minHeight: 0 }}>
      ...
      <div style={{
        background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12,
        overflow: 'auto', flex: 1, minHeight: 240,
      }}>
